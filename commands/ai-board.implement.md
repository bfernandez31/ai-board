...
4. **Project Setup Verification**:
   - **REQUIRED**: Create/ai-board.verify ignore files based on actual project setup:
   
   **Detection & Creation Logic**:
   - Check if the following command succeeds to determine if the repository is a git repo (create/ai-board.verify .gitignore if so):

     ```sh
     git rev-parse --git-dir 2>/dev/null
     ```
   - Check if Dockerfile* exists or Docker in plan.md → create/ai-board.verify .dockerignore
   - Check if .eslintrc* or eslint.config.* exists → create/ai-board.verify .eslintignore
   - Check if .prettierrc* exists → create/ai-board.verify .prettierignore
   - Check if .npmrc or package.json exists → create/ai-board.verify .npmignore (if publishing)
   - Check if terraform files (*.tf) exist → create/ai-board.verify .terraformignore
   - Check if .helmignore needed (helm charts present) → create/ai-board.verify .helmignore
