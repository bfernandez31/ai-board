...
# Check if PR already exists FIRST (handles re-verify after rollback)
echo ""
echo "🔍 Checking if PR already exists for branch: ${BRANCH}..."

EXISTING_PR_URL=$(gh pr view "${BRANCH}" --json url -q '.url' 2>/dev/null || echo "")

if [ -n "$EXISTING_PR_URL" ]; then
  echo "✅ PR already exists: ${EXISTING_PR_URL}"
  echo "   Skipping PR creation (this is expected after rollback + re-verify)"
...
### Testing
✅ All tests passed in the verify workflow
...
🤖 Generated with [Claude Code](https://claude.com/claude-code)" \
  ...
  if [ $PR_CREATE_EXIT -eq 0 ]; then
    ...
    echo "❌ PR creation failed (exit code: $PR_CREATE_EXIT)"
