...
export async function handleTicketTransition(
  ...
  console.log('[Transition] About to enqueue workflow for job', job.id, 'command:', command);
  try {
    ...
    console.log('[Transition] Workflow enqueued successfully for job', job.id);
    return {
      success: true,
      jobId: job.id,
    ...
    console.error('[Transition] Failed to enqueue workflow:', error);
