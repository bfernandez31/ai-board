 ...
 */
async function processBatchPayload(body: unknown, startTime: number): Promise<TelemetryProcessResult> {
  if (typeof body === 'object' && body && 'agent' in body && body.agent === 'GEMINI') {
    return {
      status: 400,
      body: { error: 'Gemini batch payloads are no longer supported; send native OTLP logs instead' },
    };
  }
  ...
  if (!validationResult.success) {
    ...
    return { status: 400, body: { error: 'Invalid batch payload' } };
  }
...
/**
 * Main entrypoint: process a parsed telemetry request body and persist job metrics.
 *
 * Handles three input shapes:
 * - OTLP metrics envelope (resourceMetrics/resource_metrics) — ignored, accepted as 200
 * - Batch payload (no OTLP envelope) — pre-aggregated metrics for Mistral
 * - OTLP logs envelope (resourceLogs/resource_logs) — Claude/Codex/Gemini events
 */
export async function processTelemetry(
  ...
  if (body && typeof body === 'object' && (
    (body as Record<string, unknown>).resourceMetrics ||
    (body as Record<string, unknown>).resource_metrics
  )) {
    return {
      status: 200,
      body: { status: 'accepted', message: 'Metrics ignored at logs endpoint' },
    };
  }
  ...
  if (
    body &&
    typeof body === 'object' &&
    !(body as Record<string, unknown>).resourceLogs &&
    !(body as Record<string, unknown>).resource_logs &&
    !(body as Record<string, unknown>).resourceMetrics &&
    !(body as Record<string, unknown>).resource_metrics
  ) {
  ...
  let normalized: Record<string, unknown> = body as Record<string, unknown>;
  if (normalized && !normalized.resourceLogs && normalized.resource_logs) {
    normalized = normalizeOtlpKeys(normalized) as Record<string, unknown>;
  }

  // Validate OTLP schema
  const validationResult = otlpLogsSchema.safeParse(normalized);
  if (!validationResult.success) {
    console.error('[OTLP Telemetry] Schema validation failed:', {
      errors: validationResult.error.issues,
      receivedKeys: Object.keys(normalized || {}),
      bodyPreview: JSON.stringify(normalized).slice(0, 500),
    });
    return { status: 400, body: { error: 'Invalid OTLP format' } };
  }
  ...
  for (const resourceLog of otlpData.resourceLogs) {
    ...
    if (jobIdAttr && !jobId) {
      const parsed = parseInt(String(jobIdAttr), 10);
      if (isNaN(parsed)) {
