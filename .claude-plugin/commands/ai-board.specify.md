...
```bash
claude "/ai-board.specify {...}" ticket-assets/123/mockup.png ticket-assets/123/diagram.png
```

## Outline

The text the user typed after `/ai-board.specify` in the triggering message **is** the feature description. Assume you always have it available in this conversation even if `$ARGUMENTS` appears literally below. Do not ask the user to repeat it unless they provided an empty command.
...
2. Run the script to create the branch:
   - If `TICKET_KEY` is set (JSON mode from ai-board):
     `${CLAUDE_PLUGIN_ROOT:-./.claude-plugin}/scripts/bash/create-new-feature.sh --json --ticket-key="$TICKET_KEY" "$TITLE"`
   - Else (CLI mode):
     `${CLAUDE_PLUGIN_ROOT:-./.claude-plugin}/scripts/bash/create-new-feature.sh --json "$FEATURE_DESCRIPTION"`
...
3. Load `${CLAUDE_PLUGIN_ROOT:-./.claude-plugin}/templates/spec-template.md` to understand required sections.
