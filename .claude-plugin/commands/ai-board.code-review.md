...
4. Then, launch 5 parallel Sonnet agents to independently code review the change. **Each agent MUST return both a list of issues AND a dimension quality score (0-100) for their area of review.** The dimension score should reflect the overall quality of the PR in that dimension (100 = excellent, no issues; 0 = critical problems). The agents should do the following, then return a list of issues, the reason each issue was flagged, and their dimension score:
   ...
   d. Agent #4 (Spec Sync, weight: 0.00): Check whether any modified `specs/specifications/**/*.md` files in the pull request stay consistent with the code changes in the same pull request. Look only at spec files modified in this PR, not the whole specs tree. Flag contradictions and obvious gaps where specs describe behavior absent from the code or code adds behavior not reflected in those modified specs. If no files matching `specs/specifications/**/*.md` are modified in the PR, return score `100` and no issues. **Return a `dimensionScore` (0-100) for spec sync quality.**
...
5. **Quality Score Consolidation**: After all 5 agents complete, collect their dimension scores and compute the weighted final quality score. **Print the quality score JSON as a single line to stdout**, prefixed with the marker `QUALITY_SCORE_JSON:`. This allows the verify workflow to parse the score from the agent's output. The JSON structure is:
   ...
   The `qualityScore` is `round(sum(dimension.score * dimension.weight))` across the active weighted dimensions. `Spec Sync` MUST still be included in `dimensions`, but because its weight is `0.00`, it does not affect `qualityScore` or the threshold.
