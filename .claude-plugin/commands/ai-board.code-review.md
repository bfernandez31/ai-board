...
4. Then, launch 5 parallel Sonnet agents to independently code review the change. The agents should do the following, then return a list of issues and the reason each issue was flagged (eg. CLAUDE.md adherence, constitution compliance, bug, historical git context, etc.):
   a. Agent #1: Audit the changes to make sure they comply with the CLAUDE.md **and** the constitution file (`${CLAUDE_PLUGIN_ROOT:-./.claude-plugin}/memory/constitution.md`). The constitution contains non-negotiable rules that MUST be followed. Note that CLAUDE.md is guidance for Claude as it writes code, so not all instructions will be applicable during code review.
