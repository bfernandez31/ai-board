...
# /ai-board.verify - Test Failure Resolution Command
...
```bash
/ai-board.verify tests/api/tickets.spec.ts tests/e2e/board.spec.ts
```
...
- 💡 **Your job**: Determine if failure is a bug OR intentional behavior change
...
- **Decision Framework**:
  ...
  3. If spec requires new behavior → Update test to match spec
  4. If implementation doesn't match spec → Fix implementation
  5. When unclear → Default to fixing implementation (safer)
...
+   where: { projectId, stage: 'INBOX' }  // ← ACCIDENTAL FILTER!
+ })
...
Step 4: Decision
→ Fix implementation: Remove the accidental stage filter
→ Do NOT fix test - test expectations match specification
```
...
Step 4: Decision
→ Update test: Change assertion to expect new pagination format
→ Do NOT fix implementation - it correctly implements the spec
```
...
- ✅ **All Pass**: Commit all fixes, exit successfully (workflow creates PR)
- ❌ **Some Fail**: Report remaining failures with analysis, exit with error (workflow stops)
...
# - If test expects old behavior, spec documents new → Update test
```
...
- ❌ Disable tests to make suite pass
- ❌ Make superficial fixes without understanding root cause
- ❌ Increase timeouts excessively without justification
- ❌ Mark complete if any tests still failing
