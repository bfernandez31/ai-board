...
- [X] T034 ✅ DONE [P] Run `bun run test:unit` and `bun run test:integration`; confirm all new/extended suites pass. NOTE: re-implemented after a BUILD→PLAN rollback wiped the code (spec preserved). Impacted unit + component suites pass this session: job-update-validator (8 cases incl. tokenSavingOutcome), token-saving-badge (3), run-settings-dialog (7), ticket-detail-modal (28 incl. kebab=3-items + badge visibility). type-check + lint clean. Integration suites (token-saving.test.ts, transitions.test.ts) still could NOT execute in this sandbox — confirmed again: the Next.js dev server boots but Turbopack throws `Maximum call stack size exceeded` loading the `@prisma/client` external module, so every API route 500s. Environment-level (Prisma loads fine via `prisma generate`/`node`, migration applied cleanly). They are written and ready to run in CI.
