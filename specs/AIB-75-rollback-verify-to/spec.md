...
- **Decision**: Rollback should only be available when the latest workflow job is FAILED or CANCELLED (not COMPLETED, RUNNING, or PENDING)
- **Policy Applied**: CONSERVATIVE (via AUTO fallback)
- **Confidence**: Low (score -1, |absScore| = 1) - AUTO detected internal/workflow keywords (-2) but neutral feature context (+1), resulting in low confidence requiring CONSERVATIVE fallback
- **Fallback Triggered?**: Yes - confidence < 0.5 triggered CONSERVATIVE fallback per AUTO policy rules
- **Trade-offs**:
  1. Users cannot rollback successfully completed VERIFY jobs (prevents accidental undoing of verified work)
  2. Requires explicit job failure/cancellation before rollback is available
- **Reviewer Notes**: Verify this aligns with existing BUILD to INBOX rollback behavior (which also requires FAILED/CANCELLED status). Consider if users should be able to rollback completed verifications in exceptional cases.
...
### User Story 1 - Rollback Failed Verification to Re-Plan (Priority: P1)

A developer has moved a ticket through SPECIFY, PLAN, BUILD, and VERIFY stages, but the verification job failed (tests didn't pass, or implementation has issues). The developer wants to go back to PLAN stage to reconsider the implementation approach, effectively starting fresh from the planning phase.

**Why this priority**: This is the core functionality - enabling users to recover from failed verification by returning to planning stage with implementation work reverted.

**Independent Test**: Can be fully tested by dragging a VERIFY-stage ticket with FAILED job status to PLAN column and verifying the ticket resets appropriately.

**Acceptance Scenarios**:

1. **Given** a ticket in VERIFY stage with workflowType=FULL and most recent job status=FAILED, **When** user drags the ticket to PLAN column, **Then** confirmation modal appears explaining the rollback will reset implementation changes.
...
3. **Given** rollback is confirmed, **When** transition completes, **Then** failed/cancelled job is deleted from the database and ticket version is incremented.

---

### User Story 2 - Prevent Accidental Rollback (Priority: P2)

A developer accidentally drags a VERIFY ticket toward the PLAN column. The system should prevent unintentional rollback of verified work and only allow rollback when explicitly confirmed.
...
1. **Given** a ticket in VERIFY stage with most recent job status=COMPLETED, **When** user drags the ticket to PLAN column, **Then** drop zone appears disabled/blocked and rollback is not allowed.

2. **Given** a ticket in VERIFY stage with a RUNNING job, **When** user drags the ticket to PLAN column, **Then** drop zone appears disabled with message indicating job must complete first.
...
1. **Given** user starts dragging a VERIFY ticket with FAILED job, **When** hovering over PLAN column, **Then** column shows amber/red dashed border indicating rollback is available.

2. **Given** user starts dragging a VERIFY ticket with COMPLETED job, **When** hovering over PLAN column, **Then** column shows disabled styling (opacity reduced, cursor not-allowed).
...
- **FR-002**: System MUST allow VERIFY to PLAN transition only when the most recent workflow job is FAILED or CANCELLED
...
- **FR-006**: System MUST delete the FAILED/CANCELLED job record when rollback completes
...
- **Job**: Most recent non-AI-BOARD job determines rollback eligibility; FAILED/CANCELLED jobs deleted on successful rollback
