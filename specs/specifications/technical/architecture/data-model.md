...
model Job {
  ...
  // Agent telemetry metrics (aggregated from all API calls in the job)
...
- `updatedAt`: Last modification timestamp
- `inputTokens`: Total input tokens consumed by agent API calls (nullable)
- `outputTokens`: Total output tokens generated by the agent (nullable)
- `thinkingTokens`: Total thinking/reasoning tokens used (nullable; Gemini thinking mode only)
- `cacheReadTokens`: Total cache read tokens (nullable)
- `cacheCreationTokens`: Total cache creation tokens (nullable)
- `costUsd`: Total cost in USD for agent API calls (nullable, float); estimated server-side for Codex, Mistral, and Gemini
- `durationMs`: Total duration of agent API calls in milliseconds (nullable)
- `model`: Primary agent model used (max 50 chars, nullable)
- `toolsUsed`: Array of agent tools used during execution (default: empty array)
