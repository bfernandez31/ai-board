...
Encrypted storage for user-provided AI provider API keys (BYOK — Bring Your Own Key).
...
**Purpose**: Store one encrypted API key per user per AI provider for workflow use

**Fields**:
- `id`: Auto-incrementing unique identifier
- `userId`: Credential owner (required foreign key)
- `provider`: AI provider enum (`ANTHROPIC`)
- `credentialType`: Credential format (`API_KEY` | `OAUTH_TOKEN`)
- `label`: User-assigned label (max 100 chars)
- `encryptedKey`: AES-256-GCM ciphertext (hex-encoded)
- `iv`: 12-byte initialization vector (hex-encoded, 24 chars)
- `authTag`: 16-byte GCM authentication tag (hex-encoded, 32 chars)
- `preview`: Last 4 characters of the plaintext key for UI identification
...
**Constraints**:
- Unique (userId, provider) — one credential per provider per user; POST upserts in place
- Index on userId for efficient listing

**Business Rules**:
- Keys are encrypted with AES-256-GCM using `BYOK_ENCRYPTION_KEY` before storage
- Plaintext key is never stored or returned via user-facing APIs
- Key is decrypted only at workflow runtime via `POST /api/workflows/credentials` (workflow-token auth only)
- Upsert semantics: saving a new key for the same provider replaces the previous one
...
Supported AI providers for BYOK credentials.
...
Format of the stored credential.

```prisma
enum CredentialType {
  API_KEY     // Standard API key (e.g., sk-ant-...)
...
├── apiCredentials (one-to-many) → ApiCredential
