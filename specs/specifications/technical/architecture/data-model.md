...
model Job {
  ...
  agentType           String?   @db.VarChar(20)  // Agent type from OTLP service.name (e.g. claude-code, codex)
...
- `outputTokens`: Total output tokens generated by agent (nullable)
- `cacheReadTokens`: Total cache read tokens (nullable)
- `cacheCreationTokens`: Total cache creation tokens (nullable)
- `costUsd`: Total cost in USD for agent API calls (nullable, float)
- `durationMs`: Total duration of agent API calls in milliseconds (nullable)
- `model`: Primary model used (max 50 chars, nullable)
- `agentType`: Agent type derived from OTLP `service.name` attribute (max 20 chars, nullable; e.g. `claude-code`, `codex`)
- `toolsUsed`: Array of tools used during execution (default: empty array)
