...
- **Protocol**: Claude Code, Codex, and Gemini export telemetry via OTLP HTTP JSON to `/api/telemetry/v1/logs`; Mistral sends normalized batch JSON after command execution
- **Metrics per Job**: Input/output/cached tokens, cost or unavailable-cost state, duration, model, tools used
- **Cost Tracking**: Real cost reported by Claude agent; estimated from provider pricing tables for Codex, Gemini, and Mistral when supported; unsupported Gemini models preserve usage with unavailable cost
...
- **Batching**: Claude Code uses `OTEL_LOGS_EXPORT_INTERVAL=60000` (default 5s), Codex uses `OTEL_BLRP_SCHEDULE_DELAY=60000` (Rust SDK default 1s), and Gemini uses the same OTLP batch processor settings for native `gemini_cli.*` log exports; Mistral remains post-execution batch telemetry
