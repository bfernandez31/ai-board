...
Comparison endpoints cover two storage layers: file-based markdown reports (generated by the `/compare` command on feature branches) and database-backed structured records (saved via workflow API for visual dashboard rendering). Both coexist — file-based reports remain for backward compatibility, DB records power the visual dashboard.

### POST /api/projects/:projectId/comparisons

Save a new structured comparison record after the `/compare` command completes analysis.
...
{
  "sourceTicketId": 42,
  "recommendation": "Ticket AIB-100 provides the most comprehensive implementation with superior test coverage.",
  ...
  "entries": [
    {
      ...
      "score": 87.5,
      "isWinner": true,
      "keyDifferentiators": "Best test coverage (0.85 ratio), full constitution compliance",
      "linesAdded": 450,
      "linesRemoved": 30,
      "sourceFileCount": 12,
      "testFileCount": 8,
  ...
  "decisionPoints": [
    {
      ...
      "verdict": "Ticket AIB-100's structured error responses are superior",
      "approaches": "{\"AIB-100\":{\"approach\":\"try-catch with Zod\",\"assessment\":\"Follows constitution rules\"}}"
    }
  ]
}
```

**Validation** (Zod): `entries` requires minimum 2 items; all numeric fields are non-negative; `score` is 0–100; `testRatio` is 0–1; `complianceData` and `approaches` are JSON strings.
...
```json
{
  ...
  "entries": [
    { "id": 1, "ticketId": 42, "rank": 1, "score": 87.5, "isWinner": true },
    { "id": 2, "ticketId": 43, "rank": 2, "score": 72.0, "isWinner": false }
  ]
}
```

**Errors**:
- `400`: Zod validation failure (malformed payload)
- `401`: Missing or invalid Bearer token
- `404`: Project or ticket not found
- `422`: Business rule violation (e.g., tickets not in same project, no winner marked)

---

### GET /api/projects/:projectId/comparisons

List all DB-backed comparisons for a project.
...
Get a full comparison record with enriched data (ticket metadata, telemetry, quality scores aggregated from existing tables).
  ...
  "decisionPoints": [
    {
      ...
      "approaches": {
        "AIB-100": { "approach": "try-catch with Zod", "assessment": "Follows constitution rules" }
      }
    }
  ]
}
...
**Enrichment sources**:
- `ticket`: Ticket table (id, ticketKey, title, stage, workflowType, branch)
- `telemetry`: Aggregated from Job table (sum costUsd, durationMs, tokens for COMPLETED jobs); `null` when no jobs
- `qualityScore`: Latest COMPLETED verify Job's qualityScore/qualityScoreDetails; `null` when unavailable (UI shows "Pending")

**Errors**:
- `401`: Not authenticated
- `403`: Not project owner or member
- `404`: Project or comparison not found

---

### GET /api/projects/:projectId/tickets/:id/comparisons/db

List all DB-backed comparisons a specific ticket participates in (as any entry, not just source).
...
Lightweight existence check for DB-backed comparisons. Used to conditionally render the Comparisons tab in the ticket detail modal.
...
**Response** (200 OK):
```json
{ "hasComparisons": true, "count": 2 }
```

**Empty state**:
```json
{ "hasComparisons": false, "count": 0 }
```
...
**Performance**: <100ms (indexed lookup only). Cached 30s by TanStack Query.

---

### File-Based Comparison Endpoints

The following endpoints serve file-based markdown comparison reports generated by the `/compare` command and stored on feature branches.

### GET /api/projects/:projectId/tickets/:id/comparisons

Fetch list of comparison reports for a ticket.
...
```json
{
  "comparisons": [
    {
      "filename": "20260102-143000-vs-AIB-124-AIB-125.md",
    ...
    {
      "filename": "20260101-120000-vs-AIB-120.md",
      "generatedAt": "2026-01-01T12:00:00.000Z",
      "sourceTicket": "AIB-123",
      "comparedTickets": ["AIB-120"],
      "alignmentScore": 42,
      "isAligned": true
    }
  ],
...
- `sourceTicket`: Ticket key where comparison was requested
- `comparedTickets`: Array of ticket keys that were compared
- `alignmentScore`: Feature alignment percentage (0-100), 0 if not yet parsed from file
- `isAligned`: Boolean indicating if alignment >= 30%
...
**Empty State**:
- Returns empty array when no comparisons exist
- Returns empty array when ticket has no branch
- Returns empty array in test mode
...
**Performance**: <1s for typical use cases (listing comparisons directory)

### GET /api/projects/:projectId/tickets/:id/comparisons/check

Quick check if a ticket has comparison reports (used for UI button visibility).
...
```json
{
  "hasComparisons": true,
  "count": 3,
  "latestReport": "20260102-143000-vs-AIB-124-AIB-125.md"
}
```

**Fields**:
- `hasComparisons`: Boolean indicating if any comparisons exist
- `count`: Total number of comparison reports
- `latestReport`: Filename of most recent comparison (null if none)
...
**Performance**: <300ms (optimized for quick UI checks)

**Usage**:
- Called when rendering ticket detail modal
- Determines if "Compare" button should be visible
- Cached by TanStack Query for performance

### GET /api/projects/:projectId/tickets/:id/comparisons/:filename

Fetch content of a specific comparison report.
...
- `projectId` (number, required): Project ID
- `id` (number, required): Ticket ID
- `filename` (string, required): Report filename (e.g., "20260102-143000-vs-AIB-124-AIB-125.md")

**Response** (200 OK):
```json
{
  "content": "# Ticket Comparison Report\n\n## Executive Summary...",
  "metadata": {
    "path": "specs/AIB-123-feature/comparisons/20260102-143000-vs-AIB-124-AIB-125.md",
    "branch": "AIB-123-feature",
    "filename": "20260102-143000-vs-AIB-124-AIB-125.md",
    "sha": "a1b2c3d4e5f6",
    "size": 12456,
    "generatedAt": "2026-01-02T14:30:00.000Z",
...
**Filename Validation**:
- Must match pattern: `{timestamp}-vs-{KEYS}.md`
- Timestamp format: `YYYYMMDD-HHMMSS`
- Keys format: Dash-separated ticket keys (e.g., `AIB-124-AIB-125`)

**Errors**:
- `400`: Invalid project, ticket ID, or filename format
- `401`: Not authenticated
- `403`: User is neither project owner nor member, or ticket belongs to different project
- `404`: Project, ticket, or comparison file not found
- `500`: GitHub API error
