...
- `value`: Required, format validated per provider and type:
  - Anthropic API key: `/^sk-ant-api\d{2}-[A-Za-z0-9_-]{80,}$/`
...
**Errors**:
- `400`: Invalid credential format (`{ "error": "Invalid Anthropic API key format" }` or `{ "error": "Invalid OpenAI API key format. Key must start with \"sk-\"." }`)
- `401`: Not authenticated
...
- `provider` (string, optional): `ANTHROPIC` or `OPENAI` — defaults to `ANTHROPIC`

**Server-side behavior**:
1. Verify workflow token
2. Validate `projectId` is a positive integer and `provider` is a valid enum value (Zod schema)
...
- `401`: Missing or invalid workflow token
- `404`: `{ "error": "No ANTHROPIC credential configured for project owner. Please add your API key in Settings → AI Credentials." }` (provider name is dynamic)
