...
Fetch aggregated analytics data for a project.
...
```json
{
  "summary": {
    "totalCostUsd": 12.45,
    "costTrendPercent": 15.3,
    "successRatePercent": 87.5,
    "avgDurationMs": 180000,
    "ticketsShippedThisMonth": 8
  },
  "costOverTime": [
    {
  ...
  "cacheEfficiency": {
    "efficiencyPercent": 34.8,
    "cacheReadTokens": 80000,
    "freshInputTokens": 150000,
...
**Summary Metrics**:
- `totalCostUsd`: Total cost from all jobs in last 30 days
- `costTrendPercent`: Percentage change vs previous 30 days (null if no previous data)
- `successRatePercent`: (COMPLETED / (COMPLETED + FAILED + CANCELLED)) * 100 (null if no terminal jobs)
- `avgDurationMs`: Average execution time of completed jobs (null if no completed jobs)
- `ticketsShippedThisMonth`: Count of tickets in SHIP stage updated in current calendar month

**Cost Over Time**:
- Daily aggregation of job costs for last 30 days
- Sorted chronologically (oldest to newest)
- Only includes jobs with completedAt timestamp

**Cost By Stage**:
- Aggregated by workflow stage using COMMAND_TO_STAGE mapping
- Sorted descending by cost (highest first)
- Percentage calculated as (stage cost / total cost) * 100
- Stages: SPECIFY, PLAN, BUILD, VERIFY

**Token Usage**:
- Aggregated from all jobs in last 30 days
- inputTokens: Fresh tokens sent to API
- outputTokens: Tokens generated by model
- cacheReadTokens: Tokens served from prompt cache
- cacheCreationTokens: Tokens stored in cache
- totalTokens: Sum of all token types

**Top Tools**:
- Top 10 most frequently used Claude Code tools
- Sorted descending by usage count
- Percentage calculated as (tool usage / total tool calls) * 100
- Aggregated from Job.toolsUsed JSON arrays

**Cache Efficiency**:
- efficiencyPercent: (cacheReadTokens / (inputTokens + cacheReadTokens)) * 100
- Shows percentage of input tokens served from cache vs fresh
- 0% means no cache usage, 100% means all tokens from cache

**Workflow Distribution**:
- Distribution of ticket workflow types (FULL, QUICK, CLEAN)
- Sorted descending by percentage
- Aggregated from all project tickets (not just last 30 days)

**Velocity**:
- Weekly count of tickets moved to SHIP stage
...
**Performance**: Analytics data aggregated on-demand (no caching). Target response time <2s for projects with 500 jobs.
