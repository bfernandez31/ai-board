...
Persist comparison data generated by the `/compare` command to the database.

**Authentication**: Workflow token (Bearer, via `validateWorkflowAuth`)
**Authorization**: Workflow token only — session auth is not accepted

**Path Parameters**:
- `projectId` (number, required): Project ID
- `id` (number, required): Source ticket ID
  ...
  "participants": [
    {
      "id": 124,
      "ticketKey": "AIB-124",
      "title": "Participant ticket title",
      "stage": "VERIFY",
      "workflowType": "FULL",
      "agent": "CLAUDE"
    }
  ],
  "branch": "AIB-123-feature-branch",
  "report": {
    "metadata": {
      "generatedAt": "2026-03-21T12:00:00.000Z",
      "sourceTicket": "AIB-123",
      "comparedTickets": ["AIB-124"],
      "filePath": "20260321-120000-vs-aib-124.md"
    },
...
**Notes**:
- `testCoverage` in implementation records is optional
- `participants` must contain at least one entry
- `agent` on ticket refs is nullable

**Response** (201 Created):
```json
{
  "id": 42,
  "markdownPath": "comparisons/20260321-120000-vs-aib-124.md"
}
```

**Errors**:
- `400`: Invalid project/ticket ID, invalid JSON, or schema validation failure
- `401`: Missing or invalid workflow token
- `403`: Ticket does not belong to the specified project
- `404`: Ticket not found
