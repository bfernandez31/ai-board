...
sequenceDiagram
    ...
    alt AGENT_TYPE = CLAUDE
        ...
        RS->>RS: configure OTEL env vars (OTEL_EXPORTER_OTLP_ENDPOINT)
...
| Telemetry | Env vars (passed through from workflow) | `~/.codex/config.toml` with `[otel]` section | Post-execution batch JSON via `collect_mistral_telemetry()`; datalake disabled | Env vars (passed through from workflow) |
