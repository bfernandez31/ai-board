...
Each workflow job captures agent usage metrics via OTLP telemetry. Claude Code, Codex, and Gemini CLI agents send telemetry to the same endpoint using their respective event name prefixes (`claude_code.*`, `codex.*`, and `gemini_cli.*`).
...
- Claude sends event names in `body.stringValue` (e.g., `claude_code.api_request`)
- Codex sends event names in `attributes[event.name]` (e.g., `codex.sse_event`)
- Gemini sends event names in `body.stringValue` (e.g., `gemini_cli.api_response`, `gemini_cli.tool_call`)
...
- Cache read tokens (from prompt caching, Claude and Gemini)
- Cache creation tokens (for new cached content, Claude only)
- Thought tokens (reasoning tokens reported by Gemini)
...
- Gemini does not report cost directly; cost is estimated from Google AI pricing based on token counts (falls back to gemini-2.5-pro rates for unknown models):

| Model | Input (per 1M tokens) | Output (per 1M tokens) | Cached Input (per 1M tokens) |
|-------|----------------------|------------------------|------------------------------|
| gemini-2.5-pro | $1.25 | $10.00 | $0.3125 |
| gemini-2.5-flash | $0.15 | $3.50 | $0.0375 |
...
**Gemini Export Configuration**:
- Gemini CLI uses native OTLP export via `GEMINI_TELEMETRY_ENABLED=1`, `OTEL_EXPORTER_OTLP_ENDPOINT`, and `OTEL_EXPORTER_OTLP_PROTOCOL=http/json`
- Telemetry is batched and exported at approximately 60-second intervals
- Telemetry collection is fire-and-forget — export failures never cause job failures
...
**Scope**:
- CLAUDE, CODEX, and MISTRAL are available for all workflow types: SPECIFY, PLAN, BUILD, VERIFY, QUICK, AI-BOARD assist, iterate
- GEMINI is available for speckit (SPECIFY/PLAN/BUILD), quick-impl, and iterate workflows only — it is not supported for verify, ai-board-assist, retro-spec, or onboard (these have hardcoded Claude dependencies)
