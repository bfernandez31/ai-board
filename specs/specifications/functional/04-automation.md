...
Each workflow job captures agent usage metrics via OTLP telemetry. Claude Code, Codex, and Gemini agents send telemetry to the same endpoint using their respective event name prefixes (`claude_code.*`, `codex.*`, and `gemini_cli.*`).
...
- Codex and Gemini send event names in `attributes[event.name]` (e.g., `codex.result` or `gemini_cli.api_response`)
...
- Cache creation tokens (for new cached content, Claude and Gemini)
...
- Codex, Gemini, and Mistral do not report cost directly; cost is estimated from provider pricing based on token counts:

| Provider | Model | Input (per 1M tokens) | Output (per 1M tokens) | Cached Input (per 1M tokens) |
|----------|-------|----------------------|------------------------|------------------------------|
| OpenAI | gpt-5-codex | $1.25 | $10.00 | $0.625 |
| OpenAI | gpt-5.3-codex | $1.75 | $14.00 | $0.875 |
| Google | gemini-2.0-pro | $1.25 | $3.75 | $0.625 |
| Google | gemini-1.5-flash | $0.075 | $0.30 | $0.0375 |

**Performance**:
- Total duration in milliseconds
  - Claude and Gemini report `duration_ms` per API request; values are summed across all telemetry batches
...
- Primary model used (e.g., claude-sonnet-4-5, gpt-5-codex, gemini-2.0-pro)
...
- Only log export is enabled for Codex and Gemini agents (traces and metrics export are disabled)
- Telemetry needs are fully served by structured log records containing token counts and event data
