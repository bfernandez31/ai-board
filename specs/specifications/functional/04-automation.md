...
Each workflow job captures agent usage metrics. Claude Code and Codex agents send OTLP telemetry to the endpoint using their respective event name prefixes (`claude_code.*` and `codex.*`). Mistral and Gemini agents use post-execution batch JSON collection.
...
- Thinking/reasoning tokens (Gemini thinking mode only — tracked and priced separately)
...
- Codex does not report cost directly; cost is estimated server-side from OpenAI API pricing:
...
- Gemini does not report cost directly; cost is estimated server-side from the Gemini pricing table (thinking tokens are priced at their own rate):

| Model | Input (per 1M tokens) | Output (per 1M tokens) | Thinking (per 1M tokens) | Cached (per 1M tokens) |
|-------|----------------------|------------------------|--------------------------|------------------------|
| gemini-2.5-pro | $1.25 | $10.00 | $3.75 | $0.3125 |
| gemini-2.5-flash | $0.15 | $0.60 | $0.45 | $0.0375 |
| gemini-2.0-flash | $0.10 | $0.40 | $0.00 | $0.025 |

- Model name matching for Gemini uses prefix matching to handle version suffixes (e.g., `gemini-2.5-pro-preview-05-06` matches `gemini-2.5-pro`)
- If a Gemini model is not in the pricing table, cost is set to null (unavailable) — no incorrect estimate is generated
...
- Total duration in milliseconds
  ...
  - Gemini reports duration in stream-json output; parsed by `collect_gemini_telemetry()`
- Primary model used (e.g., claude-sonnet-4-5, claude-opus-4-6, gpt-5-codex, gemini-2.5-flash)
...
**Gemini Telemetry Collection**:
- Gemini CLI outputs stream-json during execution; `collect_gemini_telemetry()` in `run-agent.sh` parses this file after the job completes
- Extracted fields: input tokens, output tokens, thinking tokens, cache read/creation tokens, duration, model, tool names
- Posted to the telemetry endpoint as a batch payload with `agent: "GEMINI"`
