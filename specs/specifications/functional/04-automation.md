...
Each workflow job captures Claude usage metrics:

**Token Usage**:
- Input tokens consumed by Claude API calls
- Output tokens generated by Claude responses
- Cache read tokens (from prompt caching)
- Cache creation tokens (for new cached content)

**Cost Tracking**:
- Total cost in USD for all Claude API calls in the job
...
- Metrics are aggregated across all Claude commands in a single job
