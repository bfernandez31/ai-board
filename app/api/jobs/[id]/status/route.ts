...
export async function PATCH(
  ...
  try {
    ...
    if (isTerminalState && updatedJob.startedAt && updatedJob.completedAt && (updatedJob.durationMs == null || updatedJob.durationMs === 0)) {
      const wallClockMs = updatedJob.completedAt.getTime() - updatedJob.startedAt.getTime();
      if (wallClockMs > 0) {
        await prisma.job.update({
          where: { id: jobId },
          data: { durationMs: wallClockMs },
        });
      }
    }
    ...
    if (isTerminalState) {
      ...
      // Auto-transition: on COMPLETED dispatch next stage if autoMode is on,
      // on FAILED/CANCELLED turn autoMode off so the user can intervene.
      await handleAutoTransitionAfterJob(
        jobId,
        requestedStatus as 'COMPLETED' | 'FAILED' | 'CANCELLED'
      ).catch((err) => {
        console.error('[Job Status Update] Auto-transition error:', err);
      });
    }
