...
export async function GET(
  ...
  try {
    ...
    if (!ticket.branch) {
      return NextResponse.json({ error: 'Plan not available', code: 'BRANCH_NOT_ASSIGNED', message: 'Ticket does not have a branch assigned' }, { status: 404 });
    }

    if (ticket.jobs.length === 0) {
      return NextResponse.json({ error: 'Plan not available', code: 'NOT_AVAILABLE_YET', message: 'Ticket does not have a completed "plan" job' }, { status: 404 });
    }
    ...
    try {
      ...
      return NextResponse.json({
        ...
        metadata: {
          ...
          docType: 'plan', fileName: 'plan.md',
          filePath: `specs/${ticket.branch}/plan.md`,
          fetchedAt: new Date().toISOString(),
        },
      });
    } catch (error) {
      console.error('GitHub API error fetching plan:', error instanceof Error ? error.message : error);
      if (error instanceof Error) {
        ...
        if (error.message.includes('not found')) return NextResponse.json({ error: 'Plan file not found', code: 'FILE_NOT_FOUND', message: `File does not exist at specs/${ticket.branch}/plan.md` }, { status: 404 });
      }
      return NextResponse.json({ error: 'Failed to fetch plan', code: 'GITHUB_API_ERROR' }, { status: 500 });
    }
  } catch (error) {
