...
export async function GET(
  ...
  try {
    ...
    const ticket = await prisma.ticket.findFirst({
      ...
      include: {
        jobs: { where: { command: 'specify', status: 'COMPLETED' }, take: 1 },
        project: true,
      },
    });
    ...
    if (!ticket.branch) {
      return NextResponse.json({ error: 'Specification not available', code: 'SPEC_NOT_AVAILABLE', message: 'Ticket does not have a branch assigned' }, { status: 404 });
    }

    if (ticket.jobs.length === 0) {
      return NextResponse.json({ error: 'Specification not available', code: 'SPEC_NOT_AVAILABLE', message: 'Ticket does not have a completed "specify" job' }, { status: 404 });
    }
    ...
    try {
      ...
      return NextResponse.json({
        ...
        metadata: {
          ...
          docType: 'spec', fileName: 'spec.md',
          filePath: `specs/${ticket.branch}/spec.md`,
          fetchedAt: new Date().toISOString(),
        },
      });
    } catch (error) {
      console.error('GitHub API error fetching spec:', error instanceof Error ? error.message : error);
      if (error instanceof Error) {
        ...
        if (error.message.includes('not found')) return NextResponse.json({ error: 'Specification file not found', code: 'FILE_NOT_FOUND', message: `File does not exist at specs/${ticket.branch}/spec.md` }, { status: 404 });
      }
      return NextResponse.json({ error: 'Failed to fetch specification', code: 'GITHUB_API_ERROR' }, { status: 500 });
    }
  } catch (error) {
