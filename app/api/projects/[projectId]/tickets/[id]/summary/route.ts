 ...
 *
 * Retrieves the summary.md file for a ticket from GitHub.
 *
 * @param request - Next.js request object
 * @param context - Route context with projectId and ticket id params
 *
 * @returns JSON response with summary content and metadata
 *
 * @throws 400 - Invalid project or ticket ID
 * @throws 403 - Ticket belongs to different project
 * @throws 404 - Project, ticket, or summary file not found
 * @throws 500 - GitHub API error or internal server error
 *
 * @example
 * GET /api/projects/1/tickets/123/summary
 * Response: { content: "# Summary...", metadata: {...} }
 */
...
export async function GET(
  ...
  try {
    ...
    const ticket = await prisma.ticket.findFirst({
      ...
      include: {
        jobs: {
          where: { command: 'implement', status: 'COMPLETED' },
          take: 1,
        },
        project: true,
      },
    ...
    // Check if ticket has completed implement job
    const hasCompletedImplementJob = ticket.jobs.length > 0;
    if (!hasCompletedImplementJob) {
      console.error('Ticket has no completed implement job:', {
        ticketId,
        projectId,
      ...
      return NextResponse.json(
        {
          error: 'Summary not available',
          code: 'SUMMARY_NOT_AVAILABLE',
          message: 'Ticket does not have a completed "implement" job',
        },
        { status: 404 }
      );
    }
    ...
    // Fetch summary content from GitHub
    try {
          ...
          return NextResponse.json(
            {
              ...
              code: 'FILE_NOT_FOUND',
              message: `File does not exist at specs/${ticket.branch}/summary.md`,
            },
            { status: 404 }
          );
        }
