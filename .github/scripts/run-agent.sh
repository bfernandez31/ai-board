...
install_gemini() {
  ...
  if ! npm install -g @google/gemini-cli@latest >&2; then
...
invoke_gemini() {
  ...
  log_info "Invoking Gemini with command file: $command_file (Native OTLP enabled)"
  log_info "Prompt file: $prompt_file ($(wc -c < "$prompt_file") bytes)"

  # Native OTLP configuration for Gemini CLI
  if [[ -n "${OTEL_EXPORTER_OTLP_ENDPOINT:-}" ]]; then
    export OTEL_EXPORTER_OTLP_ENDPOINT="${OTEL_EXPORTER_OTLP_ENDPOINT}/v1/logs"
    export OTEL_RESOURCE_ATTRIBUTES="job_id=${JOB_ID:-0}"
    log_info "Gemini OTLP endpoint: $OTEL_EXPORTER_OTLP_ENDPOINT"
  fi
