...
# Placeholders in .env.test are written as \${VAR} so dotenv-expand (used by
# Next.js when loading .env.test) does not recurse on self-references. We
# match the literal "\${VAR}" here so sed replaces the full escaped form.
