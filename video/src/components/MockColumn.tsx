...
export const MockColumn: React.FC<{ label: string; color: string; count: number; children?: React.ReactNode; style?: React.CSSProperties }> = ({ label, color, count, children, style }) => {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', width: 260, flexShrink: 0, ...style }}>
      ...
      <div style={{ background: `${color}08`, flex: 1, padding: 10, display: 'flex', flexDirection: 'column', gap: 8, borderRadius: '0 0 12px 12px', minHeight: 300 }}>
        {children}
      </div>
    </div>
  );
