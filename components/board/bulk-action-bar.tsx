...
const BULK_LIMIT = 50;
const SUCCESS_INDICATOR_MS = 1800;
...
export interface BulkActionBarProps {
  ...
  /** Monotonically-increasing tick set by the parent each time a non-destructive bulk update succeeds. */
  agentSuccessTick?: number;
  modelSuccessTick?: number;
}
...
export function BulkActionBar({
  ...
  const [agentSuccessVisible, setAgentSuccessVisible] = React.useState(false);
  const [modelSuccessVisible, setModelSuccessVisible] = React.useState(false);

  React.useEffect(() => {
    if (agentSuccessTick === 0) return;
    setAgentSuccessVisible(true);
    const timer = setTimeout(() => setAgentSuccessVisible(false), SUCCESS_INDICATOR_MS);
    return () => clearTimeout(timer);
  }, [agentSuccessTick]);

  React.useEffect(() => {
    if (modelSuccessTick === 0) return;
    setModelSuccessVisible(true);
    const timer = setTimeout(() => setModelSuccessVisible(false), SUCCESS_INDICATOR_MS);
    return () => clearTimeout(timer);
  }, [modelSuccessTick]);
      ...
      <div className="relative flex items-center">
        ...
        {modelSuccessVisible ? (
          <span
            role="status"
            aria-live="polite"
            data-testid="bulk-model-success"
            className="ml-1 inline-flex items-center gap-1 text-emerald-600 dark:text-emerald-400"
          >
            <Check className="h-4 w-4" />
            <span className="sr-only">Model updated</span>
          </span>
        ) : null}
      </div>
