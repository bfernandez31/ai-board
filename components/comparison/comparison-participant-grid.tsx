...
function ParticipantCard({
  ...
  participant: ComparisonParticipantGridProps['participants'][number];
}): React.JSX.Element {
...
export function ComparisonParticipantGrid({
  participants,
}: ComparisonParticipantGridProps): React.JSX.Element | null {
  if (participants.length === 0) return null;

  return (
    <div className="flex flex-wrap gap-3">
      {participants.map((participant) => (
        <ParticipantCard key={participant.ticketId} participant={participant} />
      ))}
    </div>
  );
}
