...
import type { ProjectNavigationDestination } from '@/lib/types';

interface DesktopProjectRailProps {
  projectId: number;
}

function getRailLinkClassName(isActive: boolean): string {
  return cn(
    ...
    return (
      <ProjectRailTooltip key={destination.id} label={destination.label}>
        <Link
          ...
          className={getRailLinkClassName(destination.isActive)}
        >
    ...
    >
      <nav className="sticky top-16 flex h-[calc(100vh-4rem)] w-14 flex-col items-center py-4">
        <div className="flex w-full flex-col items-center gap-2">
          {renderDestinationLinks(primaryDestinations)}
        </div>

        <div className="mt-auto flex w-full flex-col items-center gap-2">
          {renderDestinationLinks(footerDestinations)}
        </div>
      </nav>
    </aside>
  );
